...
mkdir -p "\$ROOT/cloudflare" "\$ROOT/zapier" "\$ROOT/stripe"
...
(cd "\$ROOT/cloudflare" && npm install --silent "wrangler@$CloudflareVersion")
(cd "\$ROOT/zapier" && npm install --silent "@zapier/zapier-sdk-cli@$ZapierVersion")
(cd "\$ROOT/stripe" && npm install --silent "stripe@$StripeSdkVersion")
